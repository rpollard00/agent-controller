...
public sealed partial class LocalGitSourceControlProvider : ISourceControlProvider
{
    ...
    )
    {
        ...
        var resolution = spec.Profile is null
            ? null
            : RepositoryCloneTransportResolver.Resolve(spec.Profile, spec.RepositoryConnection);
    ...
    )
    {
        ...
        var resolution = spec.Profile is null
            ...
            : RepositoryCloneTransportResolver.Resolve(spec.Profile, spec.RepositoryConnection);
        var transport = resolution?.Transport ?? ResolveTransport(spec.Transport, cloneUrl);
        var credentialSource = resolution?.CredentialSource ?? RepositoryCloneCredentialSource.None;
        var credentialReference = resolution?.CredentialReference;

        ClonePreflightResult Failed(ClonePreflightFailureCode code, string reason)
        {
            Log.PreflightFailed(_logger, spec.RepoKey, transport, reason);
            return ClonePreflightResult.Failed(
                transport,
                cloneUrl,
                reason,
                code,
        ...
        {
            return Failed(
                ClonePreflightFailureCode.InvalidCloneUrl,
                $"Clone URL is empty for repository '{spec.RepoKey}'."
            );
        }

        if (resolution is { IsReady: false })
        {
            return Failed(
                ClonePreflightFailureCode.TransportConfigurationInvalid,
        ...
        {
            // file:// URLs are checked by git below; bare paths can fail more clearly here.
            if (
                !cloneUrl.StartsWith("file://", StringComparison.OrdinalIgnoreCase)
                && !Directory.Exists(cloneUrl)
            )
            {
                return Failed(
                    ClonePreflightFailureCode.InvalidCloneUrl,
                    $"Local repository path does not exist: '{cloneUrl}'."
                );
            }
        }
        ...
        if (
            ...
            && transport is CloneTransport.HttpsPat or CloneTransport.Ssh
            && _credentialResolver is null
        )
        {
            return Failed(
                ClonePreflightFailureCode.CredentialUnavailable,
                "Clone credential resolution is not configured on this host."
            );
        }

        GitAskPassCredentials? httpsCredentials = null;
        GitSshCredentials? sshCredentials = null;
        try
        {
            if (transport == CloneTransport.HttpsPat)
            {
                httpsCredentials = await CreateHttpsCredentialsAsync(
                    spec,
                    resolution,
                    cancellationToken
                );
            }
            else if (transport == CloneTransport.Ssh)
            {
                sshCredentials = await CreateSshCredentialsAsync(
                    spec,
                    resolution,
                    cancellationToken
                );
        ...
        catch (RepositoryCloneCredentialException ex)
        {
            return Failed(MapCredentialFailure(ex.Failure), ex.Message);
        }
        catch (InvalidOperationException)
        {
            return Failed(
                ClonePreflightFailureCode.CredentialInvalid,
                $"{GitClonePreflightProbe.DescribeCredential(resolution)} could not be prepared. Verify the stored credential material."
            );
        }
        catch (ArgumentException)
        {
            return Failed(
                ClonePreflightFailureCode.CredentialInvalid,
                $"{GitClonePreflightProbe.DescribeCredential(resolution)} contains invalid credential material."
            );
        }
        catch (Exception ex) when (ex is IOException or UnauthorizedAccessException)
        {
            return Failed(
                ClonePreflightFailureCode.CredentialUnavailable,
                $"Temporary material for {GitClonePreflightProbe.DescribeCredential(resolution)} could not be created on this host."
            );
        }

        using (httpsCredentials)
        using (sshCredentials)
        {
            var probe = await GitClonePreflightProbe.RunAsync(
                cloneUrl,
                spec.DefaultBranch,
                transport,
                resolution,
                disableCredentialHelpers: httpsCredentials is not null,
            ...
            {
                return Failed(
                    probe.FailureCode ?? ClonePreflightFailureCode.RemoteRejected,
                    probe.Reason
                );
            }
        }
        ...
        return ClonePreflightResult.Ok(transport, cloneUrl, credentialSource, credentialReference);
    }

    private static ClonePreflightFailureCode MapCredentialFailure(
        ...
        failure switch
        {
            ...
            RepositoryCloneCredentialFailure.TypeMismatch =>
                ClonePreflightFailureCode.CredentialTypeMismatch,
            RepositoryCloneCredentialFailure.InvalidPayload =>
                ClonePreflightFailureCode.CredentialInvalid,
            RepositoryCloneCredentialFailure.StoreUnavailable =>
                ClonePreflightFailureCode.CredentialUnavailable,
            _ => ClonePreflightFailureCode.CredentialUnavailable,
        };
    ...
    /// </summary>
    internal static CloneTransport ResolveTransport(
        CloneTransport explicitTransport,
        string cloneUrl
    ...
    internal static string RemoveCredentialsFromCloneUrl(string cloneUrl)
    {
        ...
        var sanitized = new UriBuilder(uri) { UserName = string.Empty, Password = string.Empty };
    ...
    /// </summary>
    internal static readonly Dictionary<string, string?> GitNonInteractiveEnv = new(
