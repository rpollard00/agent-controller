<script lang="ts">
  ...
  let {
    ...
    environments: RuntimeEnvironmentProfile[];
    empty: boolean;
    updatingKey?: string;
    onrefresh: () => void;
...
<Card
  title="Managed runtime environments"
  description="Workspace providers and agent runtimes configured through Agent Controller."
>
  {#snippet actions()}
  ...
  {#if empty}
    <div class="rounded-xl border border-dashed border-slate-700 px-5 py-12 text-center">
      <h2 class="font-semibold text-white">No runtime environments yet</h2>
      <p class="mx-auto mt-2 max-w-lg text-sm leading-6 text-slate-400">
        Add an environment to configure where agent workspaces and runtimes execute.
      </p>
      <a
  ...
  {:else}
    <DataTable caption="Managed runtime environments">
          ...
          <tr class="align-top">
            <th class="px-4 py-4 font-medium" scope="row">
              <a
                ...
                href={runtimeEnvironmentDetailPath(profile.key)}
              >
                {profile.displayName}
              </a>
            ...
            <td class="px-4 py-3">
              <div class="flex min-w-max justify-end gap-1">
                ...
                <a
                  href={runtimeEnvironmentEditPath(profile.key)}
